...
class ProductTechnicalTabContentBlockController extends BlockController {
	
	protected $btName = 'Product Technical tab content';
	protected $btDescription = '';
	protected $btTable = 'btDCProductTechnicalTabContent';
	...
	public function getSearchableContent() {
		return $this->field_3_wysiwyg_content;
	}
...
//WYSIWYG HELPER FUNCTIONS (COPIED FROM "CONTENT" BLOCK):
	function translateFromEditMode($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('ProductTechnicalTabContentBlockController', 'replaceFileIDInEditMode'),
			$text);


		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('ProductTechnicalTabContentBlockController', 'replaceDownloadFileIDInEditMode'),
			$text);
	...
	function translateFrom($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:CID_([0-9]+)}/i',
			array('ProductTechnicalTabContentBlockController', 'replaceCollectionID'),
			$text);

		$text = preg_replace_callback(
			'/<img [^>]*src\s*=\s*"{CCM:FID_([0-9]+)}"[^>]*>/i',
			array('ProductTechnicalTabContentBlockController', 'replaceImageID'),
			$text);

		// now we add in support for the files that we view inline
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('ProductTechnicalTabContentBlockController', 'replaceFileID'),
			$text);
		...
		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('ProductTechnicalTabContentBlockController', 'replaceDownloadFileID'),
			$text);
