<?php   
	...
	class SortableResponsiveGalleryBlockController extends BlockController {
		...
		protected $btName = "Sortable Responsive Gallery";
		protected $btTable = 'btSortableResponsiveGallery';
		...
		public function on_page_view() {
			if ($this->enableLightbox) {
				...
				$this->addHeaderItem($html->css($bv->getBlockURL() . '/responsive/jquery.fancybox-1.3.1.css', null, array('handle' => 'jquery.fancybox', 'version' => '1.3.1')));
				$this->addHeaderItem($html->javascript($bv->getBlockURL() . '/responsive/jquery.fancybox-1.3.1.pack.js', null, array('handle' => 'jquery.fancybox', 'version' => '1.3.1')));
			}
		}
		
		public function view() {
			Loader::model('sortable_responsive_gallery', 'sortable_responsive_gallery');
			$sg = new SortableResponsiveGallery($this->bID);
		...
		function add() {
			...
			$this->set('thumbWidth', '300');
			$this->set('thumbHeight', '300');
		...
		private function set_tools_urls() {
			...
			$this->set('get_filesets_url', $th->getToolsURL('get_fileset_select_options', 'sortable_responsive_gallery'));
			$this->set('get_thumbnails_url', $th->getToolsURL('get_thumbnail_items', 'sortable_responsive_gallery'));
		}			
