...
class SortableResponsiveGallery extends Object {
//We're extending "Object", not "Model", because we don't want an ActiveRecord-style object
	...
	private function getSortedFileIDs($imagesOnly) {
		...
		$sql = "SELECT fsf.fID, p.position" . ($fsHasDisplayOrder ? ", fsf.fsDisplayOrder" : "")
			 ...
			 . " btSortableResponsiveGallery b INNER JOIN btSortableResponsiveGalleryPositions p ON b.bID=p.bID"
			 ...
			 . " FROM FileSetFiles fsf LEFT JOIN btSortableResponsiveGallery b ON fsf.fsID=b.fsID"
			 . " WHERE b.bID = {$this->bID}"
			 . " AND NOT EXISTS(SELECT * FROM btSortableResponsiveGalleryPositions p WHERE p.bID = {$this->bID} AND p.fID = fsf.fID)"
			 . ($imagesOnly ? " AND EXISTS (SELECT * FROM FileVersions fv WHERE fv.fID = fsf.fID AND fv.fvIsApproved = 1 AND fv.fvType = ".FileType::T_IMAGE.")" : '')
	...
	public function setPositions($sortedFileIDs) {
		...
		$sql = "DELETE FROM btSortableResponsiveGalleryPositions WHERE bID = ?";
		...
		if ($values) {
			$sql = "INSERT INTO btSortableResponsiveGalleryPositions VALUES " . implode(',', $values);
			$db->Execute($sql);
		}
	}
